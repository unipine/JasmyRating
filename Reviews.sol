...
contract Reviews {
    ...
    function addProfile(address _adrs, string memory _desc) public {
        profilesCount++;
        Profile storage p = profiles[_adrs];
        p.id = profilesCount;
        p.adrs = _adrs; // msg.sender;
        p.description = _desc;
        p.reviewsCount = 0;
    }
    ...
    function addReview(
        address _fromAdrs,
        address _profileAdrs,
        uint8 _review
    ) public {
        // _fromAdrs == msg.sender
        require(_review > 0 && _review < 6); // valid review from 1 to 5
        require(profiles[_profileAdrs].reviews[_fromAdrs] == 0); // sender can rate someone else only once

        profiles[_profileAdrs].reviews[_fromAdrs] = _review;
        profiles[_profileAdrs].reviewsCount++;
        profiles[_profileAdrs].reviewsSum += _review;
        profiles[_profileAdrs].average = uint8(
            profiles[_profileAdrs].reviewsSum /
